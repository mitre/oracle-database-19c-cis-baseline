control 'oracle19c-5.2.16' do
  title "Ensure 'GRANT ANY PRIVILEGE' Is Revoked from Unauthorized 'GRANTEE'"
  desc  "The Oracle database `GRANT ANY PRIVILEGE` keyword provides the grantee
the capability to grant any single privilege to any item in the catalog of the
database. Unauthorized grantees should not have that privilege."
  desc  'rationale', "The `GRANT ANY PRIVILEGE` capability can allow an
...
ORACLE_MAINTAINED='Y')
  ...
  describe 'Unauthorized users should not be able to grant any privilege -- list of GRANTEES with `GRANT ANY PRIVILEGE` privileges' do
    subject { parameter }
    it { should be_empty }
  end
end
