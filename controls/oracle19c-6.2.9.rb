...
restrictions. Enabling this unified action audit enables logging of all `CREATE
...
profiles, whether successful or unsuccessful, may provide clues and forensic
...
all user activities involving creation of database profiles."
  desc  'check', "
...
using the `CREATE AUDIT POLICY` statement.
  ...
  describe 'Ensure CREATE PROFILE audit option is enabled -- CREATE PROFILE Action Audit' do
    subject { parameter }
    it { should be_empty }
  end
end
