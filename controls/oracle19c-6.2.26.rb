...
unified audit causes logging of all `DROP TRIGGER` statements, whether
successful or unsuccessful, issued by the users regardless of the privileges
held by the users to issue such statements."
  desc  'rationale', "Logging and monitoring of all attempts to drop triggers,
...
using the `CREATE AUDIT POLICY` statement.
  ...
  describe 'Ensure DROP TRIGGER audit option is enabled -- DROP TRIGGER Action Audit' do
    subject { parameter }
    it { should be_empty }
  end
end
