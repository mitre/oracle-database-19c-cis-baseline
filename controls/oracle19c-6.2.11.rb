...
restrictions. Enabling this unified action audit enables logging of all `DROP
...
regardless of the privileges held by the users to issue such statements."
  desc  'rationale', "Logging and monitoring of all attempts to drop profiles,
...
involving dropping database profiles."
  desc  'check', "
...
using the `CREATE AUDIT POLICY` statement.
  ...
  describe 'Ensure DROP PROFILE audit option is enabled -- DROP PROFILE Action Audit' do
    subject { parameter}
    it { should be_empty }
  end
end
