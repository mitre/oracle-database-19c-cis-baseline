...
control 'oracle19c-6.2.2' do
  title "Ensure the 'ALTER USER' Action Audit Is Enabled"
  desc  "The `ALTER USER` statement is used to change database users’ password,
lock accounts, and expire passwords. In addition, this statement is used to
change database properties of user accounts such as database profiles, default
and temporary tablespaces, and tablespace quotas. This unified audit action
enables logging of all `ALTER USER` statements, whether successful or
unsuccessful, issued by the users regardless of the privileges held by the
users to issue such statements."
  desc  'rationale', "Logging and monitoring of all attempts to alter user
...
all activities involving `ALTER USER`."
  desc  'check', "
...
using the `CREATE AUDIT POLICY` statement.
  ...
  describe 'Ensure ALTER USER audit option is enabled -- ALTER USER Action Audit' do
    subject { parameter}
    it { should be_empty }
  end
end
