...
synonym. Enabling his unified action audit causes logging of all `DROP SYNONYM`
and `DROP PUBLIC SYNONYM` statements, whether successful or unsuccessful,
issued by the users regardless of the privileges held by the users to issue
such statements."
  desc  'rationale', "Logging and monitoring of all attempts to drop synonyms,
...
involving dropping of synonyms or public synonyms."
  desc  'check', "
...
using the `CREATE AUDIT POLICY` statement.
  ...
  describe 'Ensure DROP SYNONYM audit option is enabled -- DROP SYNONYM Action Audit' do
    subject { parameter }
    it { should be_empty }
  end
end
